...
//   ✅ Customers
//   ✅ Inventory items
//   ✅ Stock Receives → Stock Batches → Stock Movements
//   ✅ Tools
//   ✅ Labour Types & Labours
//
// Safe to run on top of existing data (uses upsert / findFirst guards).
...
async function seedCategories() {
  ...
  const subCategoryMap: Record<string, number> = {}; // key: "catName|subName"
...
async function seedInventory(
  ...
  // Use a map keyed by itemCode so downstream lookup is reliable
  ...
  for (const item of items) {
    ...
    // The DB still has a legacy NOT NULL "unit" column removed from the schema.
    // We must use raw SQL to supply it — Prisma client omits unknown fields.
    ...
    const fetched = await prisma.inventory.findUnique({
  ...
  console.log(`     ✅ ${created} inventory items created (${Object.keys(idMap).length} total resolved)`);
  return items
...
async function seedStock(
  ...
  for (const recv of receives) {
    ...
    // Create items + batches + movements
...
async function main() {
  ...
  // Resolve Super Admin user id (needed for stock movements)
  ...
  if (!adminUser) {
    throw new Error(
      "No SUPER_ADMIN user found. Please run the main seed.ts first."
    );
  }
