...
// ============================================================
// src/app/(Main)/pipe-cut-pieces/actions.ts
...
const PIPE_CUT_PATH = "/pipe-cut-pieces";
...
export interface GetPipeCutPiecesResult {
  pipeCutPieces: PipeCutPieceRow[];
  total: number;
  page: number;
  limit: number;
  totalPages: number;
...
export async function getPipeCutPieces(
...
): Promise<GetPipeCutPiecesResult> {
  ...
  const [total, pipeCutPieces] = await Promise.all([
    prisma.pipeCutPiece.count({ where }),
    prisma.pipeCutPiece.findMany({
      where,
      select: {
        id: true,
        parentLength: true,
  ...
  return {
    pipeCutPieces: pipeCutPieces as PipeCutPieceRow[],
    total,
