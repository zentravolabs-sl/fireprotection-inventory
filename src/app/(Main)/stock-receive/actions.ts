...
// ============================================================
// src/app/(Main)/stock-receive/actions.ts
...
const STOCK_RECEIVE_PATH = "/stock-receive";
...
export interface GetStockReceivesResult {
  stockReceives: StockReceiveRow[];
  total: number;
  page: number;
  limit: number;
  totalPages: number;
...
export async function getStockReceives(
...
): Promise<GetStockReceivesResult> {
  ...
  const [total, stockReceives] = await Promise.all([
    prisma.stockReceive.count({ where }),
    prisma.stockReceive.findMany({
      where,
  ...
  return {
    stockReceives: stockReceives as StockReceiveRow[],
    total,
