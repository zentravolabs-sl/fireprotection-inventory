...
export default function StatusBadge({ status, label, size = "md" }: StatusBadgeProps) {
  ...
  const sizeClasses = size === "sm" ? "px-2 py-0.5 text-[11px] min-w-[100px]" : "px-2.5 py-1 text-xs min-w-[125px]";

  return (
    <span
      className={`inline-flex items-center justify-center text-center gap-1.5 font-semibold border rounded-full ${style.bg} ${style.text} ${style.border} ${sizeClasses}`}
    >
