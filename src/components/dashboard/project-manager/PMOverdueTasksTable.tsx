...
// ============================================================
// src/components/dashboard/project-manager/PMOverdueTasksTable.tsx
// High-priority Warning Section: Overdue Tasks.
// ============================================================

import React from "react";
import { AlertTriangle, CheckCircle2 } from "lucide-react";
import { PMOverdueTaskItem } from "@/lib/services/projectManagerDashboardService";

interface PMOverdueTasksTableProps {
  tasks: PMOverdueTaskItem[];
}

export function PMOverdueTasksTable({ tasks }: PMOverdueTasksTableProps) {
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-red-200 dark:border-red-900/60 p-6 shadow-sm flex flex-col justify-between h-full bg-gradient-to-br from-red-50/10 to-transparent dark:from-red-950/10">
      <div>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
          Tasks past target deadline requiring immediate project manager intervention
        </p>
        ...
        {tasks.length === 0 ? (
          <div className="py-8 text-center text-xs text-emerald-600 dark:text-emerald-400 font-semibold bg-emerald-50/50 dark:bg-emerald-950/20 rounded-xl border border-emerald-100 dark:border-emerald-900/40 flex items-center justify-center gap-2">
            <CheckCircle2 size={16} /> Great! There are no overdue tasks.
          </div>
        ) : (
              ...
              >
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-gray-500 dark:text-gray-400">
                    {t.projectName} • <span className="font-semibold text-gray-700 dark:text-gray-300">{t.assignedEngineer}</span>
                  </p>
                </div>
