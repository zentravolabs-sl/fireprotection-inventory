...
// ============================================================
// src/components/dashboard/engineer/EngMaterialRequestsTable.tsx
// Engineer's Requisition Log for Site Materials.
// ============================================================

import React from "react";
import Link from "next/link";
import { Package, PlusCircle, ArrowRight } from "lucide-react";
import { EngMaterialRequestItem } from "@/lib/services/engineerDashboardService";

interface EngMaterialRequestsTableProps {
  requests: EngMaterialRequestItem[];
}
...
export function EngMaterialRequestsTable({ requests }: EngMaterialRequestsTableProps) {
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 shadow-sm flex flex-col justify-between h-full">
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-1">
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="px-3 py-1.5 text-xs font-bold text-white bg-amber-600 hover:bg-amber-700 rounded-xl transition-colors shadow-xs flex items-center gap-1"
            >
              <PlusCircle size={12} /> Create Request
            </Link>
            <Link
              href="/material-requests"
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
          Material requisitions submitted for site installations
        </p>
